...
try {
	$result = $fnName([
		...
		'formData' => function () {
			// Simulate FormData
			// This is tricky because JS FormData API is object-oriented.
			// We return a simple wrapper or just the raw arrays?
			// The PHP action code generated by us (from TS) likely expects RequestEvent.
			// But our conversion logic (regex) kept the signature simple?
			// Actually, the regex replaced \`action_foo(event)\` with \`prefix_action_foo(event)\`.
			// So we need to pass an event object.
			return $_POST; // Simplified
		}
	],
		'url' =>(object)[
	'searchParams' =>(object)$_GET
...
${mode === "php-static" ? `
// 2. Serve PHP files if they exist
$candidates = [];

// Case A: /path -> /path.php
$candidates[] = rtrim($path, '/') . '.php';

// Case B: /path/ -> /path/index.php
if (is_dir($path)) {
    $candidates[] = rtrim($path, '/') . '/index.php';
}
...
foreach ($candidates as $candidate) {
    if ($uri !== '/' && file_exists($candidate)) {
        ...
        if ($requested_file && str_starts_with($requested_file, $build_dir)) {
            router_log("Serving PHP file: $candidate");
...
if (!$served) {
    router_log("Candidates checked: " . implode(', ', $candidates) . " (Exists: NO)");
}
...
if (file_exists($path)) {
    if (is_dir($path)) {
        if (file_exists($path . '/index.php')) {
            ...
            router_log("Serving Directory Index: $file");
            $_SERVER['SCRIPT_FILENAME'] = $file;
            require $file;
            return;
        } else {
            router_log("Directory exists but no index.php: $path");
        }
    }
} else {
    router_log("Path does not exist: $path");
}
` : `
...
`)[0];
          ...
          const dataPhp = getDataPhp(includes, builder.config.kit.paths.base).replace("PLACEHOLDER_ROUTE_ID", JSON.stringify(navPath)).replace("PLACEHOLDER_TEMPLATE_JSON", templateJson).replace("PLACEHOLDER_LOAD_FNS", loadFnsPhp).replace("PLACEHOLDER_INLINE_MODE", JSON.stringify(inlineMode));
