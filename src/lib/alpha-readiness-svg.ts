...
export function renderAlphaReadinessSvg(
	...
	return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="title desc" data-alpha-proof-ledger="proofLedger" data-native-platform-provenance="lg-ultragear-native-platform-provenance" data-native-host-handoff-controls="native-window-action" data-required-alpha-evidence="requiredEvidence">
	<title id="title">SvelteKit PHP ${escapeSvg(report.target)} alpha readiness graphic</title>
	<desc id="desc">Native-styled alpha readiness graphic with readiness scores, community analytics, hosted smoke status, sourceToKeywordEdge links, analyticsLinkageMarker metadata, weightedDemandScore, freshnessMaxAgeHours, trustBoundary, manualReviewRequired, required-alpha-evidence markers ${escapeSvg(requiredEvidenceMarkers)}, progressReportHandoff statusMapping, ProgressBarStatus.Indeterminate, lg-ultragear-native-platform-provenance markers, and proofLedger markers ${escapeSvg(proofLedgerMarkers)} with blocker status ${escapeSvg(proofLedgerBlockers)}.</desc>
	<metadata>requiredEvidence required-alpha-evidence ${escapeSvg(requiredEvidenceMarkers)} proofLedger ${escapeSvg(proofLedgerMarkers)} ${escapeSvg(proofLedgerBlockers)} alpha-over-rc-release-policy alpha-runtime-gate-ledger hosted-php-smoke-proof-required needs-local-gate-proof needs-hosted-proof analyticsLinkageMarker=${escapeSvg(analyticsLinkageMarkers)} sourceToKeywordEdge=count:${escapeSvg(sourceToKeywordEdgeCount)} weightedDemandScore=${escapeSvg(weightedDemandScore)} freshnessMaxAgeHours=${escapeSvg(freshnessWindows)} trustBoundary=${escapeSvg(trustBoundaries)} manualReviewRequired=true lg-ultragear-native-platform-provenance desktopShellUiBinding @scriptgpt/desktop-shell-ui packages/desktop-shell-ui/src/index.ts installSvelteKitPhpNativeHost enableMicaWindowChrome syncTaskbarProgress toggleWindowMaximize Effect.Mica ProgressBarStatus.Indeterminate win.setEffects win.startDragging win.setProgressBar reportJson reportUrl data-native-host-handoff-controls set-window-effect set-progress clear-progress report-ready setWindowEffect setProgress clearProgress reportReady</metadata>
	<defs>
	...
	<text x="186" y="84" class="chrome-label">Windows 11 Mica + macOS traffic lights</text>
	...
	<text x="78" y="626" class="small">Evidence trust: deterministic-local-artifact, directional-community-signal, deterministic-runtime-evidence</text>
	...
	<text x="620" y="692" class="small">Chrome proof: native-visual-matrix, windows-mica-visual-row, macos-traffic-light-row, data-window-control, native-window-action</text>
	...
	<text x="620" y="736" class="small">Native provenance: lg-ultragear-native-platform-provenance, @scriptgpt/desktop-shell-ui, desktopShellUiBinding, enableMicaWindowChrome, syncTaskbarProgress</text>
