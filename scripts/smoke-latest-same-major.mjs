...
async function assertLatestSameMajorTargets() {
	const versions = {};
	for (const [packageName, expectedMajor] of Object.entries(expectedMajors)) {
		...
		if (actualMajor !== expectedMajor) {
			throw new Error(
				`${packageName}@${version} is not same-major with the alpha lane (${expectedMajor}.x). Update the audit before claiming same-major latest support.`
			);
		}
...
async function writeFixture(tempRoot, tarballPath, packageJson, versions) {
	...
	await writeFile(
		...
		`${JSON.stringify(
			{
				name: 'sveltekit-php-latest-same-major-smoke',
				private: true,
				type: 'module',
				scripts: {
					build: 'vite build'
	...
	await writeFile(
		...
		[
			'<svelte:head>',
			'  <title>latest same-major smoke</title>',
			'</svelte:head>',
			'',
			'<main data-latest-same-major-smoke="sveltekit-php">',
			'  <h1>latest same-major smoke</h1>',
			'  <p>adapter build output verified with latest Svelte 5 and SvelteKit 2.</p>',
			'</main>',
			''
		].join('\n')
	);
...
async function assertBuildOutput(tempRoot, versions) {
	...
	if (!html.includes('data-latest-same-major-smoke="sveltekit-php"')) {
		throw new Error('Generated index.php is missing latest same-major fixture marker.');
	}
	...
	console.log(
		`PASS latest-same-major-smoke: built php-static fixture with svelte@${versions.svelte} and @sveltejs/kit@${versions['@sveltejs/kit']}.`
	);
	console.log('PASS latest-same-major-smoke: output includes index.php, .htaccess, router.php, compat.php, and route manifest.');
}

async function main() {
	const packageJson = JSON.parse(await readFile(path.join(repoRoot, 'package.json'), 'utf8'));
	const versions = await assertLatestSameMajorTargets();
	const exportShape = await assertPackageExportShape();
	const packManifest = await createAndAssertPackagePack(exportShape);
	const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'sveltekit-php-latest-same-major-'));
	...
	try {
		...
		if (!keep) {
			...
			console.log(`Kept latest same-major temp directory: ${tempRoot}`);
		}
	}
}
