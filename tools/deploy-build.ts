...
async function run(cmd: string, args: string[], envOverride?: Record<string, string>): Promise<number> {
...
async function buildManifest(localRootAbs: string, mode: Mode, progress: boolean): Promise<Manifest> {
...
async function loadManifest(p: string): Promise<Manifest | null> {
  try {
    ...
    if (j?.version === 1 && j?.files) {
      const files: Record<string, ManifestFile> = {};
      for (const [k, v] of Object.entries(j.files as Record<string, any>)) {
        files[k] = {
          path: v.path ?? k,
...
async function fullUploadArchive(o: Options, curr: Manifest): Promise<void> {
  ...
  const remoteKeepQ = remoteKeepRaw.replace(/"/g, '\\"');
...
`;
  ...
  code = await run("scp", ["-P", String(o.port), cleanupLocal, `${sshTarget(o)}:${remoteCleanupRaw}`]);
  if (code !== 0) process.exit(code);

  code = await run("scp", ["-P", String(o.port), runnerLocal, `${sshTarget(o)}:${remoteRunnerRaw}`]);
  if (code !== 0) process.exit(code);
...
function buildPlanLines(o: Options, shouldFull: boolean, d: ReturnType<typeof diffManifests>, curr: Manifest): string[] {
  ...
  if (shouldFull) {
    ...
    lines.push(`${kind}  ${path.resolve(o.local)}${path.sep}.  ->  ${sshTarget(o)}:${o.remote.replace(/\/+$/, "")}/`);
    ...
    if (o.wipeRemote) lines.push(`WIPE  ${sshTarget(o)}:${o.remote.replace(/\/+$/, "")}/* (requires --yes-wipe)`);
    return lines;
  }
...
async function main() {
  ...
  const thresholdFiles = parseNum(a["threshold-files"] ?? env("DEPLOY_THRESHOLD_FILES"), DEFAULTS.thresholdFiles);
  const thresholdBytes = parseNum(a["threshold-bytes"] ?? env("DEPLOY_THRESHOLD_BYTES"), DEFAULTS.thresholdBytes);
  ...
  const cleanupMaxSecs = parseNum(a["cleanup-max-secs"] ?? env("DEPLOY_CLEANUP_MAX_SECS"), DEFAULTS.cleanupMaxSecs);
  const cleanupStallSecs = parseNum(a["cleanup-stall-secs"] ?? env("DEPLOY_CLEANUP_STALL_SECS"), DEFAULTS.cleanupStallSecs);
  ...
  console.log(`Mode:   ${o.mode}${o.mode === "fast" ? " (mtime+size, no hashing)" : " (sha256 hashing)"}`);
  ...
  console.log("");
  console.log(`Changed files: ${d.changed.length}, removed: ${d.removed.length} (ratio ${fmtPct(d.changedFileRatio)})`);
  console.log(`Changed bytes: ${fmtBytes(d.changedBytes)} / ${fmtBytes(d.totalBytes)} (ratio ${fmtPct(d.changedByteRatio)})`);
  ...
  console.log("");
  const question = o.dryRun ? "Dry run preview complete. Run the upload now?" : "Proceed with upload?";
