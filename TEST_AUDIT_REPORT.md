...
1.  **Node SSR Tests:**
    ...
    _Runs `tests/e2e/node-ssr/node-ssr.spec.ts`._
...
2.  **PHP Static Tests:**
    ...
    _Runs all tests in `tests/e2e/php-static/`._
...
3.  **Unit Tests:**
    ...
    _Runs Vitest on `tests/unit/`._

4.  **Verification Scripts:**
    ```bash
    bun run verify:php-static
    ```
    _Runs the custom `scripts/verify-php-static.mjs` (Legacy verification for static builds)._
